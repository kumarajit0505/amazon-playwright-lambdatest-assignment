...
const logger = new Logger("TC2 — Galaxy");
const { galaxy } = TEST_DATA;

test.describe("TC2 — Samsung Galaxy Shopping Cart", () => {
  test("Search for Galaxy, add to cart, open cart, and print price", async () => {
    const browser = await connectToBrowser(capabilities[1]);
    ...
    try {
      ...
      logger.info(`Setting delivery country to ${galaxy.deliveryCountry.name}`);
      await searchPage.setDeliveryCountry(galaxy.deliveryCountry.name);

      // Step 3: Search for Galaxy
      logger.info(`Searching for "${galaxy.searchQuery}"`);
      await searchPage.searchFor(galaxy.searchQuery);

      // Assert search results loaded
      expect(await page.title()).toContain(galaxy.searchQuery);
      ...
      await searchPage.clickFirstOrganicResult(galaxy.brandFilter);
      ...
      if (!deliverable) {
        logger.warn(
          `Shipping restriction detected — setting country to ${galaxy.deliveryCountry.name}`,
        );
        await productPage.setDeliveryCountry(galaxy.deliveryCountry.name);
      }
      ...
      await page.evaluate(
        (_) => {},
        `lambdatest_action: ${JSON.stringify({ action: "setTestStatus", arguments: { status: "passed", remark: `Galaxy cart test passed. Price: ${price}` } })}`,
      );
    } catch (err) {
      await page.evaluate(
        (_) => {},
